import { vendorCache } from './requireContext';

export default function vendor(Vue) {
    Object.keys(vendorCache).forEach(key => {
        const item = vendorCache[key];
        switch(key) {
            case 'UIComponents': 
                // console.log('item', item);
                Vue.use(item['index.js'].default);
                break;
            case 'components':
...
function setPlugins(Vue, cachePlugins) {
    Object.keys(cachePlugins).forEach(key => {
        // const name = key.replace('.js', '');
        Vue.use(cachePlugins[key].default);
    });
}
